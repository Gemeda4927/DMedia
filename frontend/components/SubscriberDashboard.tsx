...
export default function SubscriberDashboard({ user, profile }: SubscriberDashboardProps) {
  ...
  const stats = [
    ...
    },
    {
      icon: FiShield,
      label: 'Status',
      value: subscriptionStatus === 'active' ? 'Active' : 'Inactive',
      color: subscriptionStatus === 'active' ? 'from-green-500 to-green-600' : 'from-gray-500 to-gray-600',
      href: '/subscriptions',
    },
  ];
  ...
  return (
    <div className="space-y-8">
      ...
      <motion.div
        ...
        className="bg-gradient-to-r from-yellow-600/20 to-primary-600/20 rounded-2xl p-8 border border-yellow-500/30"
      >
          ...
          <div>
            <div className="flex items-center gap-2 mb-2">
              ...
              <h2 className="text-3xl font-bold">
                Welcome back, <span className="bg-gradient-to-r from-yellow-400 to-primary-400 bg-clip-text text-transparent">{user?.name}!</span>
              </h2>
            </div>
            <p className="text-gray-400">Enjoy premium access to exclusive Oromo content</p>
            {subscriptionStatus === 'active' ? (
              ...
              <p className="text-yellow-400 text-sm mt-2">
                Manage your subscription to unlock premium features
              </p>
            )}
          </div>
        </div>
      ...
      {subscriptionStatus === 'active' && premiumContent.length > 0 && (
        <div>
          <div className="flex items-center justify-between mb-6">
            <h3 className="text-2xl font-bold flex items-center gap-2">
              <FiStar className="w-6 h-6 text-yellow-400" />
              Premium Content
              <span className="px-2 py-1 bg-yellow-500/20 text-yellow-400 rounded-lg text-xs">Exclusive</span>
            </h3>
            <Link href="/content?premium=true" className="text-primary-400 hover:text-primary-300 text-sm flex items-center gap-1">
              View all <FiArrowRight className="w-4 h-4" />
            </Link>
          </div>
          <div className="grid md:grid-cols-3 lg:grid-cols-6 gap-4">
            {premiumContent.map((item: any, index) => (
              <motion.div
                key={item._id}
              ...
              >
                <Link
                  ...
                  className="block group relative overflow-hidden rounded-lg bg-gray-800/50 backdrop-blur-sm border border-yellow-500/30 hover:border-yellow-500/50 transition-all"
                >
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        <Link
          href="/subscriptions"
          className="group bg-gray-800/50 backdrop-blur-sm border border-gray-700/50 rounded-xl p-6 hover:border-yellow-500/50 transition-all"
        >
          <div className="flex items-center gap-4 mb-4">
            ...
            <div>
              <h3 className="text-xl font-semibold group-hover:text-yellow-400 transition-colors">Manage Subscription</h3>
              <p className="text-gray-400 text-sm">Upgrade or manage your plan</p>
            </div>
            <FiArrowRight className="w-5 h-5 text-gray-400 ml-auto group-hover:text-yellow-400 group-hover:translate-x-1 transition-all" />
          </div>
        </Link>
      </div>
